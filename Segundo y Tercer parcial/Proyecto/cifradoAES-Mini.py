# -*- coding: utf-8 -*-
"""
Created on Sat Jun 11 16:38:30 2022
...
def cifradoAESMini(archivo, clave):
    ...
    for i in range(0, len(bloques)):
        cifrado=cifrado+cifrado16Bits(bloques[i], clave)
    return cifrado

"""
prueba = 0b1100001111110000;
print(cifrado16Bits("1001110001100011", prueba))
"""
#generar llave    
key = get_random_bytes(2);
txt=input("ingrese el nombre del archivo de texto a cifrar: ")
cipherBin=cifradoAESMini(txt+".txt", key)
cipher64=binarioBase64(cipherBin)#pasar a base 64
nombreArchivo=input("Con que nombre se guardar su archivo de texto cifrado: ")
with open (nombreArchivo+".txt", "w", encoding="utf-8") as doc:#escritura del archivo cifrado en txt
    doc.write(cipher64)
    doc.close
#guardado de la llave
llave_bytes = base64.b64encode(key)#arreglo de bytes en base 64
llave = llave_bytes.decode('UTF-8') #texto en base 64   
nombreLlave=input("Con que nombre se guardar el archivo de texto que contiene su llave: ")     
with open (nombreLlave+".txt", "w", encoding="utf-8") as doc:#escritura de la llave en txt
    doc.write(llave)
    doc.close
